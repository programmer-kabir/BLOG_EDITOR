...
const Navbar = () => {
  ...
  <div
    className={`fixed text-white top-0 bottom-0 flex justify-end left-0 right-0 bg-black bg-opacity-75 z-50 transition-opacity duration-500 ease-in-out ${
      isNavOpen ? 'opacity-100 transition-opacity duration-500 ease-out ' : '  opacity-0 pointer-events-none '
    }`}
  >
    {/* Sidebar Container */}
    <div
      className={`w-9/12 border-l overflow-y-auto py-3 md:hidden h-screen transform transition-transform duration-500 ease-[cubic-bezier(0.7,0,0.3,1)] ${
